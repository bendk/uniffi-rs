...
fn trait_to_uniffi_meta(
    ...
    match export_ty {
        TraitExportType::TraitInterface(kind) => uniffi_meta::Type::Object {
            module_path,
            name,
                ...
                match path_to_type.item()? {
                    ...
                    Item::Builtin(item) => {
                        ...
                        Ok(match item {
                            ...
                            BuiltinItem::HashSet => {
                                let key = generics.resolve1(ir, cache, self)?;
                                Type::HashSet(Box::new(key))
                            }
                            BuiltinItem::Result => {
...
pub mod tests {
    ...
    fn test_resolve_compound_types() {
        ...
        assert_eq!(
            run_resolve_type(&ir, &mut cache, "types", "std::collections::HashSet<u32>"),
        ...
        assert_eq!(
            run_resolve_type(&ir, &mut cache, "types", "std::sync::Arc<u8>"),
        ...
        assert_eq!(
            run_resolve_type(
                ...
                &mut cache,
                "types",
                "std::sync::Arc<std::sync::Arc<Box<String>>>"
            ),
    ...
    fn test_resolve_type_trait_objects() {
        ...
        assert_eq!(
            run_resolve_type(
                ...
                &mut cache,
                "types::mod1",
                "dyn TraitInterfaceForeignOnly"
            ),
            Ok(Type::Trait {
                module_path: "types::mod1".into(),
                name: "TraitInterfaceForeignOnly".into(),
                export_ty: TraitExportType::TraitInterface(TraitKind::ForeignOnly),
            })
        );
        assert_eq!(
    ...
    fn test_resolve_type_with_type_aliases() {
        ...
        assert_eq!(
            run_resolve_type(&ir, &mut cache, "type_aliases", "::std::sync::Arc<Self>"),
    ...
    fn test_result_uniffi_meta() {
        ...
        assert_eq!(
            run_resolve_uniffi_meta_type(
                ...
                &mut cache,
                "types",
                "::std::sync::Arc<dyn mod1::TraitInterface>",
                None
            ),
            Ok(uniffi_meta::Type::Object {
                module_path: "types::mod1".into(),
