...
impl Ir {
    ...
    pub fn add_udl_metadata(
        ...
        let module = match self
            .crate_roots_mut()
            .find(|c| c.ident == crate_name.replace("-", "_"))
        {
            Some(m) => m,
            None => {
                return Err(Error::new_without_location(AddUdlMetadataError(format!(
                    "Invalid crate: {crate_name}"
                ))))
            }
        };
        for i in items {
...
impl CrateRoot {
    ...
    fn try_visit_modules_and_paths<'ir, E>(
        &'ir self,
        mut visitor: impl FnMut(&'ir Module, &RPath<'ir>) -> Result<(), E>,
    ) -> Result<(), E> {
