...
open class {{ cls.name_kt() }} : {{ cls.base_classes|join(", ") }} {
    ...
    override public fun {{ meth.callable.name_kt() }}({{ meth.callable.arg_list() }}): {{ meth.callable.result.return_type_kt() }} {
    ...
    companion object {
        ...
        public fun {{ cons.callable.name_kt() }}({{ cons.callable.arg_list() }}): {{ cons.callable.result.return_type_kt() }} {
            {%- let jni_method_name = cons.jni_method_name %}
            {%- let callable = cons.callable %}
            {% filter indent(8) %}{%- include "CallableBody.kt" %}{% endfilter %}
        }
