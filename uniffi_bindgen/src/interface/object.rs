...
impl Object {
    ...
    pub fn derive_ffi_funcs(&mut self, ci_namespace: &str) -> Result<()> {
        ...
        if self.ffi_func_free.name().is_empty() {
            self.ffi_func_free.name = uniffi_meta::free_fn_symbol_name(
                ci_namespace, &self.name);
        }
...
impl Constructor {
    ...
    pub fn checksum(&self) -> u16 {
        self.checksum_override.unwrap_or_else(|| uniffi_meta::checksum(self))
    }
    ...
    fn set_object_name(&mut self, ci_namespace: &str, object_name: String) {
        self.checksum_fn_name = uniffi_meta::method_checksum_symbol_name(ci_namespace, &object_name, &self.name);
...
impl Method {
    ...
    pub fn derive_ffi_func(&mut self, ci_namespace: &str, obj_name: &str) -> Result<()> {
        ...
        if self.ffi_func.name.is_empty() {
            self.ffi_func.name = uniffi_meta::method_fn_symbol_name(
                ci_namespace, obj_name, &self.name,
            );
        }
    ...
    pub fn from_meta(meta: uniffi_meta::MethodMetadata, checksum: u16) -> Self {
        ...
        Self {
            ...
            checksum_fn_name,
            checksum_override: Some(checksum),
        }
    }
}
