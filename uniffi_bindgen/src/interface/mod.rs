...
/// This should match the minor version of the `uniffi` crate.  See
...
impl ComponentInterface {
    ...
    pub fn uniffi_contract_version(&self) -> u32 {
        ...
        force_version.map(|v| v.parse().unwrap()).unwrap_or(UNIFFI_CONTRACT_VERSION)
    }
    ...
    pub fn iter_checksums(&self) -> impl Iterator<Item = (String, u16)> + '_ {
        // TODO: consolidate the checksum naming code below

        let func_checksums = self.functions.iter().map(|f| (f.checksum_fn_name(), f.checksum()));
        let method_checksums = self.objects.iter().flat_map(|o| {
            o.methods().into_iter().map(|m| (m.checksum_fn_name(), m.checksum()))
        });
        let constructor_checksums = self.objects.iter().flat_map(|o| {
            o.constructors().into_iter().map(|c| (c.checksum_fn_name(), c.checksum()))
        });
        func_checksums
    ...
    pub(super) fn add_fn_meta(&mut self, meta: FnMetadata, checksum: u16) -> Result<()> {
        self.add_function_impl(Function::from_meta(meta, checksum))
    }

    pub(super) fn add_method_meta(&mut self, meta: MethodMetadata, checksum: u16) {
        let object = get_or_insert_object(&mut self.objects, &meta.self_name);
        object.methods.push(Method::from_meta(meta, checksum));
    }
