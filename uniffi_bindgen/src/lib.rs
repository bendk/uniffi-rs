...
pub fn run_tests<P: AsRef<Path>>(
    ...
    for (lang, test_scripts) in language_tests {
        for udl_file in udl_files {
            ...
            let extra_files = if lang == TargetLanguage::Kotlin && udl_path.file_name().and_then(|f| f.to_str()) == Some("decorators.udl") {
            ...
            bindings::compile_bindings(&config.bindings, &component, &cdylib_dir, lang, &extra_files)?;
        }
        for test_script in test_scripts {
